...
switch (_type) do {
    ...
    case "ACRE_PRC152": {_unit addItem _type;};
    case "ACRE_PRC148": {_unit addItem _type;};
    ...
    case "ACE_EntrenchingTool": {
		if (({"ACE_EntrenchingTool" in items _x}count (units _group)) < 1 || groupId _group isEqualTo "P-1") then {
