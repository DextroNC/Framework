...
wedop = datetime.datetime(2018, 12, 5, 19)
satop = datetime.datetime(2018, 12, 8, 20)
...
async def time():
    ...
    print(satop)
    while(wedop < now):
        wedop += timedelta(days = 7)

    while(satop < now):
        satop += timedelta(days = 7)

    print(wedop)
    print(satop)
    weddays = wedop - now
    satdays = satop - now
    await bot.say("The next Wednesday op is in " + str(weddays.days) + " day(s), " + str(weddays.seconds//3600) + " hour(s) and " + str((weddays.seconds//60)%60) + "minute(s).")
    await bot.say("The next Saturday op is in " + str(satdays.days) + " day(s), " + str(satdays.seconds//3600) + " hour(s) and " + str((satdays.seconds//60)%60) + "minute(s).")
