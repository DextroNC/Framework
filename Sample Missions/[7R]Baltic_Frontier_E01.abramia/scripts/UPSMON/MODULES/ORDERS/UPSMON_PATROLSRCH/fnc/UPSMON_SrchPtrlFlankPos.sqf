...
private ["_npcpos","_dir2","_targetPos","_side","_typeofgroup","_scan","_points","_dist","_flankAngle","_flankdist","_roadchk","_distmin","_pool","_targetPosTemp","_terrainscan","_los_ok","_i","_final","_grp"];
				...
				if (!isnull(_grp)) then 
				{
					If (side _grp == _side) then
					{
						If (_posgrp select 0 != 0 && _posgrp select 1 != 0) then
						{
							...
							{
								_dist1 = [_posgrp,[_targetPosTemp select 0,_targetPosTemp select 1,0]] call UPSMON_distancePosSqr;
								_dist2 = [getposATL (leader _grp),[_targetPosTemp select 0,_targetPosTemp select 1,0]] call UPSMON_distancePosSqr;
								if (_dist1 > 100) then 
								{					
									_points = _points + 50;
								};
