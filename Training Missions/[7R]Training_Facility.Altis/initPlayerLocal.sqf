...
_CVA7 =  ["7R_SupplyDropAir","Call Aerial Tanker","",{null = ["STARTSPAWN",1500,"Pegasus 1-1"] spawn fw_fnc_aerialRefuel},_isLeader] call ace_interact_menu_fnc_createAction;
...
// =====================================================================================================================================================
_statement = {
	_point = getPos player;
	// Create Result Marker
	_mR = createMarkerLocal ["tnav_actual", _point];
	_mR setMarkerColorLocal "ColorBlue";
	_mR setMarkerShapeLocal "ELLIPSE";
	_mR setMarkerTypeLocal "hd_dot";
	_mR setMarkerAlphaLocal 0.6;
	_mR setMarkerSizeLocal [50,50];
	_mR setMarkerBrushLocal "Solid";
	player setPos (getPos tnav_return);
	ace_map_BFT_Enabled = true;		
	player linkItem "ItemGPS";
	player setVariable ["SR_Class", "Rifleman", true];
	"train_nav" setMarkerAlphaLocal 0;
	[{deleteMarkerLocal "tnav_actual";},[],15] call CBA_fnc_waitAndExecute;
};

// Trainee Recall
_condition = {(player getVariable ["SR_Class","Rifleman"]) isEqualTo "Nav"};
_action = ["Training_nav","Base Recall","",_statement,_condition] call ace_interact_menu_fnc_createAction;
