...
_args Params ["_target", "_spawn", "_height" ,"_callsign",["_static", false],["_track",""]];
...
private _units = [];
private _count = {
	if (_x distance2d _term < 15 && (backpack _x isEqualTo "B_Parachute")) then {_units pushBack _x; true} else {false};
...
for "_i" from 1 to (_split + 1) do {
	...
	// Call Plane
	[_targetPos, _spawnPos, _dropWP,  _end, _dir, _badge, _class,  _height, _static, _track] spawn fw_fnc_paraDropPlane;
	sleep 20;
};
